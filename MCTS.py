...
class MCTS:
    ...
    def find_leaf(self, state_int, player):
        ...
        :param player: 0 is choose chess, 1 is choose position
        '''
        states = []
        actions = []
        cur_state = state_int
        ...
        while not self.is_leaf(cur_state):
            ...
            if cur_state == state_int:
                noises = np.random.dirichlet([0.03] * game.GAME_COLS * game.GAME_ROWS)
            ...
            invalid_actions = set(range(game.GAME_COLS * game.GAME_ROWS)) - set(game.possible_position_moves(cur_state)) \
                if cur_player else set(range(game.GAME_COLS * game.GAME_ROWS)) - set(game.possible_chess_moves(cur_state))
            ...
            cur_state, won = game.move(cur_state, action)
            
            if won:
                # if somebody won the game, the value of the final state is -1 (as it is on opponent's turn)
                value = -1.0
            # check for the draw
            if value is None and len(game.possible_position_moves(cur_state)) == 0:
                value = 0.0
    ...
    def search_batch(self, count, batch_size, state_int, player, net, device="mps"):
        for _ in range(count):
            self.search_minibatch(batch_size, state_int, player, net, device)
    
    def search_minibatch(self, count, state_int, player, net, device="mps"):
        """
        ...
        if expand_queue:
            batch_v, batch_v_2 = model.state_lists_to_batch(expand_states, device)
            logits_v, values_v = net(batch_v, batch_v_2)
            ...
            for (leaf_state, states, actions), value, prob in zip(expand_queue, values, probs):
                self.visit_count[leaf_state] = [0] * game.GAME_COLS * game.GAME_ROWS
                self.value[leaf_state] = [0.0] * game.GAME_COLS * game.GAME_ROWS
                self.value_avg[leaf_state] = [0.0] * game.GAME_COLS * game.GAME_ROWS
    ...
    def get_policy_value(self, state_int, tau=1):
        ...
        if tau == 0:
            probs = [0.0] * game.GAME_COLS * game.GAME_ROWS
